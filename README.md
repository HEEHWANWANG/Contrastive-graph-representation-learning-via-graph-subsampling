...
## Project Task 
Hierarchical molecular graph representation learning via contrastive loss-based self-supervised learning. 
...
## Background 
**Subgraph Sampling for Graph Neural Networks**: Subgraph sampling은 다양한 이유로 graph neural network에 쓰인다.  
...
**Contrastive Loss**: 최근 Computer Vision에서 self-supervised learning의 방식으로 활발하게 개발되고 있는 것은 data augmentation 방법을 이용하는 방식이다.  
...
## Methods 
**Loss**: Contrastive Learning에서 많이 쓰이는 InfoNCELoss를 사용한다.[3]
...
**Graph Subsampling**: Molecular graph의 경우, 같은 molecule이라면 graph의 structure는 isomorphic하기 때문에, subgraph를 샘플링하는 과정에서는 임의의 initial node를 설정하고, initial node로부터 k-hop만큼 떨어져있는 subgraph를 샘플링하도록 하였다.  
